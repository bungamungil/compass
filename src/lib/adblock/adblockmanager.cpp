...
bool AdBlockManager::addSubscriptionFromUrl(const QUrl &url)
{
    ...
    QMessageBox::StandardButton result = QMessageBox::question(nullptr, AdBlockManager::tr("Content Blocker Subscription"), message, QMessageBox::Yes | QMessageBox::No);
...
QString AdBlockManager::elementRemoveRulesForDomain(const QUrl &url) const
{
    if (!isEnabled() || !canRunOnScheme(url.scheme()) || m_matcher->elemHideDisabledForUrl(url))
        return {};

    return m_matcher->elementRemoveRulesForDomain(url.host());
}
